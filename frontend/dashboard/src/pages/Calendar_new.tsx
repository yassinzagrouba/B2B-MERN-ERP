  ...
  const handleAddOrUpdateEvent = () => {
    ...
    if (selectedEvent) {
        ...
        prevEvents.map((event) =>
          event.id === selectedEvent.id
            ? {
                ...
                end: eventEndDate || eventStartDate, // If no end date, use start date
      ...
      const newEvent: CalendarEvent = {
        ...
        allDay: true,
        extendedProps: { calendar: eventLevel || "Primary" }, // Default to Primary if not selected
      };
      setEvents((prevEvents) => [...prevEvents, newEvent]);
    }
    
    // Refresh the calendar if necessary
              ...
              <div>
                {selectedEvent && (
                  <button
                    ...
                    type="button"
                    className="flex justify-center rounded-lg border border-red-300 bg-red-50 px-4 py-2.5 text-sm font-medium text-red-700 hover:bg-red-100 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40"
                  >
                    Delete Event
                  </button>
                )}
...
const renderEventContent = (eventInfo: any) => {
  ...
  const colorClass = colorMap[calendar] || 'bg-gray-100 border-gray-300 text-gray-800';
  
  return (
    <div
      className={`event-fc-color flex fc-event-main ${colorClass} p-1 rounded-sm border`}
    >
